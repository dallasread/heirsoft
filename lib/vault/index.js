...
var Vault = CustomElement.createElement(CONFIG, function Vault(options) {
    ...
    $el.on('submit', 'form[name="decrypt"]', function() {
        var $input = _.$(this).find('input'),
            ...
            publicKeys = _.get('vault.keys').map(function(k) { return k.public; }),
            splat = $textarea.val().trim().split(SEPARATOR),
            str = splat.length === 1 ? splat[0] : splat[1],
            data = _.get('vault').decrypt(publicKeys, [privateKey], str);
    ...
    $el.on('submit', 'form[name="encrypt"]', function() {
        var encrypted = _.get('vault').encrypt();
    ...
    $el.on('click', '[data-add-key]', function() {
        _.$(this).closest('.branch')[0].data('branch').keys.push({
            name: _.get('vault').generateKey(10),
            public: _.get('vault').generateKey(10),
            private: _.get('vault').generateKey(10),
        });
        _.update();
        return false;
    });
